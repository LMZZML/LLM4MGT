...
parser = argparse.ArgumentParser(description="reachability")
...
def translate(G, q, args):
    ...
    if args.prompt in ["CoT", "Shortpath", "Motif"] and args.basic == 1:
        with open("Graph/reachability/prompt/" + args.prompt + "-prompt.txt", "r") as f:
            exemplar = f.read()
        Q = Q + exemplar + "\n\n\n"
    ...
    Q = Q + "node " + str(q[0]) + " and node " + str(q[1]) + "?\nA:"
    # Q = Q + "node " + str(q[0]) + " and node " + str(q[1]) + "? Provide your reasoning and state the result clearly as 'yes' or 'no' at the end.\nA:"
    # Q = Q + "node " + str(q[0]) + " and node " + str(q[1]) + "? Finally, output the result as “yes” or “no.” Please do not use programming to solve this.\nA: Let's think step by step:"
...
def log(Q, res, answer, args):
    ...
    newpath = 'log/reachability/'+args.model+'-'+args.mode+'-'+time+ '-' + args.prompt
    if args.SC == 1:
        newpath = newpath + "+SC"
...
def main():
    ...
    for i in tqdm(range((g_num + batch_num - 1) // batch_num)):
        ...
        for j in range(i * batch_num, min(g_num, (i + 1) * batch_num)):
            with open("Graph/reachability/graph/" + args.mode + "/standard/graph" + str(j) + ".txt", "r") as f:
        ...
        for j in range(len(Q_list)):
            ...
            for k in range(sc):
                ...
                ans = ans.replace("**", "")
                p1 = ans.rfind("the answer is yes")
                p2 = ans.rfind("the answer is no")
                # p1 = ans.rfind("yes")
                # p2 = ans.rfind("no")
                idx = i * batch_num + j
